    ...
    if (line.startsWith("#EXTINF:")) {
      ...
      if (currentChannel) {
        ...
        if (!currentChannel.logo) {
          currentChannel.logo =
            "https://images.unsplash.com/photo-1594909122845-11baa439b7bf?w=200&auto=format&fit=crop&q=80";
        }
      ...
      if (isM3u8) {
        ...
        proxyRes.on("end", () => {
          ...
          if (
            ...
            !trimmedData.startsWith("EXTM3U") &&
            !trimmedData.startsWith("#EXTM3U")
          ) {
            console.warn(
              `Non-M3U8 response on stream endpoint: ${trimmedData.substring(0, 100)}`,
            );
            res.setHeader("Content-Type", "text/plain");
            return res
              .status(
                trimmedData.toLowerCase().includes("rate exceeded") ? 429 : 502,
              )
              .send(trimmedData);
          }
    ...
    if (!m3uText) {
      ...
      const xtreamChannels: Channel[] = streamList.map(
        (st: any, idx: number) => ({
          ...
          isPremium:
            (st.name || "").toLowerCase().includes("hd") ||
            (st.name || "").toLowerCase().includes("vip"),
          isActive: true,
