...
import { Heart, Play, Lock, Tv, LayoutGrid, List, Sparkles, Filter } from 'lucide-react';
...
export const ChannelGrid: React.FC<ChannelGridProps> = ({
  ...
  const [layoutMode, setLayoutMode] = useState<'grid' | 'toffeeList'>('toffeeList');
  const theme = THEMES[currentTheme] || THEMES.gold;

  // Calculate channel counts per category
  const channelPool = allChannels && allChannels.length > 0 ? allChannels : channels;
    ...
    <div className="flex flex-col h-full space-y-3">
      ...
      <div className="flex items-center justify-between gap-2 bg-slate-900/80 border border-slate-800/80 p-2.5 rounded-2xl backdrop-blur-sm shrink-0">
        <div className="flex items-center gap-2">
          <div className={`p-1.5 rounded-xl bg-gradient-to-r ${theme.accentGradient} text-slate-950 font-black shadow-md`}>
            <Tv className="w-4 h-4" />
          </div>
          <div>
            <span className="text-[10px] font-black uppercase text-slate-400 tracking-wider flex items-center gap-1">
              Live TV Categories <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping" />
            </span>
            <h3 className="text-xs font-black text-white flex items-center gap-1.5">
              <span>{selectedCategory}</span>
              <span className={`text-[10px] font-bold ${theme.accentText} bg-slate-950 px-2 py-0.5 rounded-full border border-slate-800`}>
            ...
            >
              ...
              <span
                className={`text-[10px] font-black px-1.5 py-0.2 rounded-md ${
                  isSelected ? 'bg-slate-950/20 text-slate-950' : 'bg-slate-950 text-slate-400 border border-slate-800'
                }`}
              >
                {count}
              </span>
              ...
              >
                ...
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  <div className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-xl bg-slate-950 border border-slate-800 overflow-hidden shrink-0 flex items-center justify-center p-2 group-hover:border-slate-700 shadow-lg transition-all">
                    ...
                    {isActive && (
                      <div className="absolute inset-0 bg-slate-950/60 backdrop-blur-[1px] flex items-center justify-center">
                        <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${theme.accentGradient} flex items-center justify-center text-slate-950 shadow-md`}>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      <span className={`px-2 py-0.5 bg-slate-950 ${theme.accentText} font-mono font-black text-xs rounded-md border border-slate-800 shrink-0`}>
                    ...
                    <div className="flex items-center gap-2 mt-1.5">
                      <span className="text-slate-400 text-xs font-semibold truncate">{channel.category}</span>
                      {isActive && (
                        <span className={`text-[10px] font-black text-slate-950 uppercase tracking-wider flex items-center gap-1 bg-gradient-to-r ${theme.accentGradient} px-2 py-0.5 rounded-md shadow-sm`}>
                          <span className="w-1.5 h-1.5 rounded-full bg-slate-950 animate-ping" /> PLAYING LIVE
                        </span>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  <button
                    ...
                    className={`p-2 rounded-xl transition-colors ${
                      isFav ? 'text-rose-500 bg-rose-500/10 border border-rose-500/20' : 'text-slate-500 hover:text-slate-300 hover:bg-slate-800'
                    }`}
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between mb-2">
                  <span className={`px-2 py-0.5 bg-slate-950 ${theme.accentText} font-mono font-black text-[10px] rounded-md border border-slate-800`}>
                    Ch. {channel.channelNumber}
                  </span>
                ...
                <div className="relative aspect-video w-full rounded-xl bg-slate-950 overflow-hidden mb-2 border border-slate-800 p-2 flex items-center justify-center group-hover:border-slate-700 transition-colors">
                  ...
                  <div className="absolute inset-0 bg-slate-950/60 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity">
                    <div className={`w-9 h-9 rounded-full bg-gradient-to-br ${theme.accentGradient} text-slate-950 flex items-center justify-center shadow-lg transform scale-75 group-hover:scale-100 transition-transform`}>
                      <Play className="w-4 h-4 fill-slate-950 ml-0.5" />
                    </div>
                  </div>

                  {isActive && (
                    <div className={`absolute bottom-1.5 right-1.5 bg-gradient-to-r ${theme.accentGradient} text-slate-950 font-black text-[8px] px-2 py-0.5 rounded-md flex items-center gap-1 shadow-md`}>
                      ...
                      style={{
                        width: `${
                          isActive
                            ...
                            : Math.min(95, Math.max(10, ((channel.channelNumber * 13 + new Date().getMinutes()) % 80) + 10))
                        }%`
                      }}
                    />
                  </div>
      ...
      {channels.length === 0 && (
        <div className="flex-1 flex flex-col items-center justify-center p-6 text-center bg-slate-900/40 border border-slate-800 rounded-3xl my-4">
          ...
          <h3 className="text-lg font-black text-white mb-1">No Channels Found</h3>
          <p className="text-xs text-slate-400 max-w-sm mb-4 leading-relaxed">
            There are no channels in this category. You can add M3U playlists or Xtream Codes servers in the Admin panel.
          </p>
          {onOpenAdmin && (
            <button
              onClick={onOpenAdmin}
              className={`px-5 py-2.5 bg-gradient-to-r ${theme.accentGradient} text-slate-950 font-black rounded-xl text-xs uppercase tracking-wider shadow-lg transition-transform active:scale-95`}
            >
              Open Admin Panel
            </button>
          )}
