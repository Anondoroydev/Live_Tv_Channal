...
export default function App() {
  ...
  useEffect(() => {
    ...
    setRecentlyWatched(prev => Array.from(new Set([activeChannel.id, ...prev])).slice(0, 10));

    apiService.fetchEPG(activeChannel.id).then((epgList) => {
  ...
  const handleSelectChannel = useCallback((channel: Channel) => {
    if (selectChannelTimer.current) {
        clearTimeout(selectChannelTimer.current);
    }
  ...
  const handlePrevChannel = useCallback(() => {
    setActiveChannel(prevActive => {
      if (!prevActive || channels.length === 0) return prevActive;
      const currentIdx = channels.findIndex(c => c.id === prevActive.id);
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case 'ArrowDown':
          e.preventDefault();
          if (isGridFocused) {
            setGridFocusedIdx(prev => Math.min(filteredChannels.length - 1, prev + 4));
          } else if (isSidebarFocused) {
            setSidebarFocusedIdx(prev => Math.min(7, prev + 1));
          }
          break;
        ...
        case 'ArrowRight':
          ...
          if (isSidebarFocused) {
            ...
            setGridFocusedIdx(prev => Math.min(filteredChannels.length - 1, prev + 1));
          }
          break;
  ...
  }, [isGridFocused, isSidebarFocused, gridFocusedIdx, sidebarFocusedIdx, channels]);

  // Filter channels based on View & Category & Search
  ...
  if (selectedCategory === 'Watchlist') {
    ...
    filteredChannels = filteredChannels.filter(c => recentlyWatched.includes(c.id));
  } else if (selectedCategory !== 'All') {
    filteredChannels = filteredChannels.filter(c => c.category.toLowerCase() === selectedCategory.toLowerCase());
  }
  ...
  return (
    <div className={`relative h-screen w-screen bg-slate-950 text-white font-sans overflow-hidden select-none`}>
      {/* Primary Blink Web TV Portal View */}
      <BlinkWebTVView
          ...
          <div className="max-w-7xl w-full mx-auto flex-1 flex flex-col">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800 mb-6 shrink-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-black text-white uppercase tracking-tight">Admin Control Panel</h2>
                  <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-0.5">Manage playlists, streams, categories, and users</p>
                </div>
              </div>
