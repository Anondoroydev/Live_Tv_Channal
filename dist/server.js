...
function classifyIsPremium(name, category = "", isDuplicate = false) {
  const lowerName = (name || "").toLowerCase();
  const lowerCat = (category || "").toLowerCase();
  if (lowerCat.includes("adult") || lowerCat.includes("18+") || lowerCat.includes("xxx") || lowerCat.includes("porn") || lowerName.includes("vip") || lowerName.includes("premium") || lowerName.includes("payperview") || lowerCat.includes("vip") || lowerCat.includes("pay per view")) {
    return true;
  }
...
var CHANNELS_CACHE_FILE = path.join(process.cwd(), "channels_cache.json");
  ...
  try {
    ...
    const proxyReq = client.get(actualUrl, options, (proxyRes) => {
      ...
      if (proxyRes.statusCode && proxyRes.statusCode >= 400) {
        ...
        return res.status(proxyRes.statusCode).send(
          proxyRes.statusCode === 429 ? "Rate Limit Exceeded (10000)" : `Stream Server Error: ${proxyRes.statusCode}`
        );
      }
      ...
      const isKnownBinaryMedia = !isM3U8ContentType && (actualUrl.match(/\.(ts|mp4|m4s|m4a|aac|mp3|flv|key|jpg|png|jpeg)(\?.*)?$/i) || actualUrl.includes("/live/") || actualUrl.includes("/movie/") || actualUrl.includes("/series/") || contentType.includes("video/") || contentType.includes("audio/") || contentType.includes("image/") || contentType.includes("octet-stream"));
      ...
      responseStream.once("data", (chunk) => {
        ...
        const isM3U8Text = isM3U8ContentType || chunkStr.startsWith("#EXTM3U") || chunkStr.startsWith("#EXT-X-") || chunkStr.includes("#EXTM3U") || chunkStr.includes("#EXTINF:") || actualUrl.match(/\.m3u8(\?.*)?$/i) !== null;
        if (isM3U8Text) {
              ...
              if (trimmed.startsWith("#")) {
                return line.replace(
                  ...
                  (_match, q1, q2, q3) => {
                    ...
                    const baseUrlObj2 = new URL(actualUrl);
                    if (!absUriObj.search && baseUrlObj2.search) {
                      absUriObj.search = baseUrlObj2.search;
                    }
    ...
    proxyReq.on("error", (err) => {
      ...
      if (isRetryable && retryCount < maxRetries && !res.headersSent) {
        ...
        const backoff = Math.min(baseDelay * Math.pow(1.5, retryCount), 2e3);
...
app.options("/api/proxy-stream", (_req, res) => {
  res.setHeader("Access-Control-Allow-Origin", "*");
  res.setHeader("Access-Control-Allow-Headers", "*");
  res.setHeader("Access-Control-Allow-Methods", "GET, HEAD, OPTIONS");
  res.sendStatus(200);
